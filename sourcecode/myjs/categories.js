...
$(function() {
  ...
  //$('.menu-sublist').on('click', '#approve', function(){
  $('.menu-sublist').on('click', '.category', function(event) {
    
    /* Prevent the default link navigation behavior. */
    event.preventDefault();

    var $menu = $(this);
    var $restaurant = $menu.parents('.restaurant-list-item');
    var $categories = $restaurant.find('.category-sublist');
   
        
    /* If the category list is shown, hide it. */
    if($categories.is(':visible')) {
      $categories.slideUp();
      if($categories.attr('id') == $menu.text())
          return;
    }
    $categories.attr('id', $menu.text());
